...
class Paralelizacion:
    ...
    def ejecutar_optimizacion(self, n_particulas=30, max_iter=100):
        ...
        tiempo = fin - inicio
        return resultados, mejor_valor, tiempo
    
def graficar_resultados(resultados, lim_inf_global, lim_sup_global, num_subregiones):
    intervalos = np.linspace(lim_inf_global, lim_sup_global, num_subregiones + 1)
    subregiones = [(intervalos[i], intervalos[i + 1]) for i in range(num_subregiones)]
    
    fig, ax = plt.subplots()
    for i, resultado in enumerate(resultados):
        posicion, valor, iteraciones = resultado
        lim_inf, lim_sup = subregiones[i]
        ax.scatter(posicion, valor, label=f'Subregión {i+1}') #({lim_inf}, {lim_sup})
...
if __name__ == '__main__':
    paralelizacion = Paralelizacion(dimensiones=1, lim_inf_global=-1000.0, lim_sup_global=1000.0, num_subregiones=6, funcion='ackley')
    resultados, mejor_valor, tiempo = paralelizacion.ejecutar_optimizacion(n_particulas=50, max_iter=100)
    print(f'Mejor valor: {mejor_valor}')
    print(f'Tiempo de ejecución: {tiempo} segundos')
    graficar_resultados(resultados, paralelizacion.lim_inf_global, paralelizacion.lim_sup_global, paralelizacion.num_subregiones)
